...
func TestSubagentLifecycleEvents_Serialization(t *testing.T) {
	start := NewSubagentStartedEvent("sub_run_1", "researcher", WithSubagentDescription("Researches topics"))
...
func TestSubagentFinishedEvent_Serialization(t *testing.T) {
	ev := NewSubagentFinishedEvent("sub_run_1", WithSubagentName("researcher"), WithSubagentSuccessOutcome())
...
func TestSubagentErrorEvent_Serialization(t *testing.T) {
	ev := NewSubagentErrorEvent("sub_run_1", "tool failed", WithSubagentErrorName("researcher"), WithSubagentErrorCode("TOOL_ERROR"))
	data, err := json.Marshal(ev)
	if err != nil {
		t.Fatalf("marshal: %v", err)
	}
...
func TestSubagentEvents_Validate(t *testing.T) {
	if err := NewSubagentStartedEvent("", "x").Validate(); err == nil {
		t.Error("expected error for empty subagentRunId")
	}
	if err := NewSubagentStartedEvent("id", "").Validate(); err == nil {
		t.Error("expected error for empty name")
	}
	if err := NewSubagentFinishedEvent("").Validate(); err == nil {
		t.Error("expected error for empty subagentRunId")
	}
	if err := NewSubagentErrorEvent("", "msg").Validate(); err == nil {
		t.Error("expected error for empty subagentRunId")
	}
	if err := NewSubagentErrorEvent("id", "").Validate(); err == nil {
		t.Error("expected error for empty message")
	}
}

func TestSubagentFinishedEvent_WithResult(t *testing.T) {
	ev := NewSubagentFinishedEvent("sub_1",
		WithSubagentName("researcher"),
		WithSubagentResult("task completed"),
		WithSubagentSuccessOutcome(),
	)
	data, err := json.Marshal(ev)
	if err != nil {
		t.Fatalf("marshal: %v", err)
...
func TestForSubagent_AttributedEvents(t *testing.T) {
	...
	base := NewEventEmitter(ch)
	sub := base.ForSubagent("sub_run_1")

	// Emit through the sub-agent wrapper.
	_ = sub.TextMessageStart("m1", new("assistant"))
	_ = sub.TextMessageContent("m1", "hello")
	close(ch)
	...
	if contentJSON == "" {
		t.Fatal("expected TEXT_MESSAGE_CONTENT event")
	}
...
func TestForSubagent_BaseEmitterUntouched(t *testing.T) {
	...
	_ = base.TextMessageContent("m1", "hello")
	// Emit through sub — should carry subagentRunId.
	_ = sub.TextMessageContent("m2", "world")
	close(ch)
