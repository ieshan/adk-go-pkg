...
| Feature | Description |
|---------|-------------|
| **OpenAI Model Provider** | Drop-in `model.LLM` adapter for any OpenAI-compatible API (OpenAI, Ollama, LiteLLM, OpenRouter, vLLM, Together AI). Supports both `/v1/chat/completions` and `/v1/responses` endpoints. |
...
- **OpenAI Responses API**: The `model/openai` package now supports both `/v1/chat/completions` and `/v1/responses` endpoints. Select via `Config.API`. The Responses path supports reasoning items (Thought:true), flat tool declarations, function_call/function_call_output items, and typed SSE streaming events. See [docs/openai-model.md](docs/openai-model.md).
